...
var postZjhm=null;
app.controller("idCardController", ["$scope", "$state", "$location","$stateParams","$rootScope",
...
function detectShowSubmitBtn(){
	...
	var img2=document.getElementById("backImage");
	if(img1.src && img2.src && img1.src.length>24 && img2.src.length>24
	 && img1.src.startsWith("data:image") && img2.src.startsWith("data:image")){
		showSubmitBtn();
	}else{
		hideSubmitBtn();
	}
